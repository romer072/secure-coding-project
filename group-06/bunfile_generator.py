...
def main():
    ...
    """
    # Change this filename when generating specific malicious test cases.
    ...
    # Malicious values for testing integer overflow in parsers that check:
    #   if (asset.data_offset + asset.data_size > header->data_section_size)
    # using unchecked unsigned arithmetic.
    #
    # UINT64_MAX - 7 + 16 wraps around to 8 in 64-bit unsigned arithmetic.
    # If the parser only checks the wrapped sum against data_section_size,
    # the range may appear valid even though the real range is impossible.
